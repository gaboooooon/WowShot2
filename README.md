...
### v1.0.6 (2026-08-18)
- キャプチャ時の通知が表示されない不具合の修正
  - 設定画面の「適用」ボタンで「キャプチャ時に通知する」「連番を記憶する」が保存されない問題を修正
  - クリップボードへのコピーが失敗した際に、以降の通知処理が行われなくなる問題を修正
    （コピーはリトライし、失敗した場合は通知にその旨を表示）
- キャプチャ処理中の想定外のエラーでアプリが終了しないよう例外処理を追加

### v1.0.5 (2026-08-18)
- Issue#2 タスクトレイの右クリックメニューに「キャプチャフォルダを開く」を追加
  - 保存先はプロファイルごとに設定できるため、複数ある場合はサブメニューから選択
  - フォルダが未作成の場合は、その旨を通知（作成はキャプチャ実行時のまま）

### v1.0.4 (2026-08-18)
- 混在DPI環境（例: 4K@150% + 2K@125%）で、キャプチャ画像の端が欠落する不具合の修正
  - プロセスのDPI認識モードを Per-Monitor V2 に変更
  - システムDPIと異なる拡大率のディスプレイで、キャプチャ範囲がズレる／拡大される問題を解消

### v1.0.3 (2026-02-01)
- Issue#1 キャプチャ設定画面を開くと常に「キャプチャ対象」が「全ディスプレイ」と表示される不具合の修正

### v1.0.2 (2026-02-01)
- メモリ解放漏れの修正
- 設定保存時のメッセージダイアログの表示削除
- タスクトレイのアイコンをダブルクリックで設定ダイアログを開くように変更
